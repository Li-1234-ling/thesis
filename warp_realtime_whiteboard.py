...
COLMAP_DIR = r"C:\Users\Administrator\Desktop\uni\MasterArbeit\Li\PixMix_test\white_board\colmap\sparse_txt\1"
MODEL_IMAGE_GLOB = r"C:\Users\Administrator\Desktop\uni\MasterArbeit\Li\PixMix_test\white_board\undist_rgb\frame_*.png"
DATABASE_PATH = r"C:\Users\Administrator\Desktop\uni\MasterArbeit\Li\PixMix_test\white_board\colmap\database.db"
VIDEO_SOURCE: Any = r"C:\Users\Administrator\Desktop\uni\MasterArbeit\Li\PixMix_test\white_board\compressed_new.mp4"
OUTPUT_VIDEO_PATH: Optional[str] = r"C:\Users\Administrator\Desktop\uni\MasterArbeit\Li\PixMix_test\white_board\output_realtime_2.mp4"
DEBUG_VIDEO_PATH: Optional[str] = r"C:\Users\Administrator\Desktop\uni\MasterArbeit\Li\PixMix_test\white_board\debug_realtime_2.mp4"

DEBUG_DIR = os.path.join("_tmp_run_output", "warp_realtime_whiteboard")
...
K_undist = np.array([
    [567.42267737714451, 0.0, 480.0],
    [0.0, 567.42267737714451, 270.0],
    [0.0, 0.0, 1.0],
], dtype=np.float64)
...
OBJECTS: List[Dict[str, Any]] = [
    {
        ...
        "ref_mask_path": "whiteboard_mask.png",
        "clean_ref_path": "clean0.png",
        "priority": 10,
    },
    # {
    #     "name": "obj2",
...
def descriptor_cache_key(model_frames: List[str]) -> str:
    parts = [
        os.path.abspath(DATABASE_PATH),
        os.path.abspath(COLMAP_DIR),
        MODEL_IMAGE_GLOB,
        str(len(model_frames)),
        os.path.basename(model_frames[0]) if model_frames else "",
        os.path.basename(model_frames[-1]) if model_frames else "",
    ]
    return hashlib.sha1("\n".join(parts).encode("utf-8")).hexdigest()


def maybe_load_descriptor_map(cache_path: str, expected_key: str) -> Optional[Dict[str, np.ndarray]]:
    if REBUILD_DESCRIPTOR_MAP or not os.path.isfile(cache_path):
        return None
    data = np.load(cache_path)
    if "source_key" not in data or str(data["source_key"]) != expected_key:
        print(f"[INFO] Ignoring stale descriptor map cache: {cache_path}")
        return None
...
def save_descriptor_map(cache_path: str, descriptors: np.ndarray, xyz: np.ndarray, pids: np.ndarray, source_key: str) -> None:
    os.makedirs(os.path.dirname(cache_path) or ".", exist_ok=True)
    np.savez(cache_path, descriptors=descriptors, xyz=xyz, pids=pids, source_key=source_key)
...
def build_descriptor_map(
    ...
    source_key = descriptor_cache_key(model_frames)
    cached = maybe_load_descriptor_map(MAP_CACHE_PATH, source_key)
    ...
    save_descriptor_map(MAP_CACHE_PATH, descriptors_arr, xyz_arr, pids_arr, source_key)
    print(
